...
import { itemVariants, stepVariants, MIN_ORDER_AMOUNT, MAX_ITEMS } from '../constants';
...
export const CustomizeStep: React.FC<CustomizeStepProps> = ({
  ...
  // Restriction checks
  const canProceed =
    totalItems > 0 && totalPrice >= MIN_ORDER_AMOUNT;
    ...
    >
      <div className="mb-4 text-center md:mb-4">
        <motion.div variants={itemVariants}>
          <h2 className="mb-2 text-2xl font-bold md:mb-3 md:text-3xl lg:text-5xl text-primary">
            Personalizá tu Kit
          </h2>
      ...
      <motion.div variants={itemVariants} className="p-4 mb-4 shadow-lg md:p-6 md:mb-4 bg-white/80 backdrop-blur-sm rounded-xl md:rounded-2xl">
            ...
            <div key={item.id} className="flex items-center justify-between p-2 rounded-lg md:p-4 bg-gray-50 md:rounded-xl">
              <div className="flex items-center space-x-2 md:space-x-3">
                <img
                  src={item.image}
                  alt={item.name}
                  className="object-contain h-10 rounded-md w-15 md:w-15 md:h-12 md:rounded-lg"
                />
                <div>
        ...
        {!canProceed && (
          <div className="mt-2 text-sm text-center text-red-600">
            {totalItems === 0
              ...
              : `El monto mínimo del pedido es $${MIN_ORDER_AMOUNT.toLocaleString()}. Actualmente: $${totalPrice.toLocaleString()}`}
          </div>
        )}
      </motion.div>
    </motion.div>
